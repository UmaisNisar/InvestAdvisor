...
/// method is a deterministic function of its inputs, so every strategy's live scorer and the
/// backtest harness share the exact same calculations and the lot is trivially unit-testable.
///
...
public static class Indicators
{
    ...
    /// relative to price". Null if ATR can't be computed.
    /// </summary>
    public static decimal? AtrPercent(IReadOnlyList<Candle> candles, int atrPeriod)
    {
        var atr = Atr(candles, atrPeriod);
