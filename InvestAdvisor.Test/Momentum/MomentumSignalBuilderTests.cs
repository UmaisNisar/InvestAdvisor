...
public class MomentumSignalBuilderTests
{
    ...
    public void Build_returns_null_without_enough_history()
    {
        ...
        static (StrategyFeatures, TradeSetup)? SmallInput(int bars) =>
            TradePlanner.Build(MomentumStrategy.Instance, 
                new StrategyInput("T", "T", "S", AssetClass.Equity, MomentumTestData.Flat(bars)),
    ...
    {
        TradePlanner.Build(MomentumStrategy.Instance, Input(MomentumTestData.Flat(120)), MomentumParams.Default).Should().BeNull();
    }

    [Fact]
    public void Build_produces_a_stop_below_and_a_target_above_the_entry()
    {
        var built = TradePlanner.Build(MomentumStrategy.Instance, Input(MomentumTestData.HighVolSqueezeBreakout()), MomentumParams.Default);
    ...
    public void Stop_and_target_track_the_ATR_multiples_so_reward_beats_risk()
    {
        ...
        var setup = TradePlanner.Build(MomentumStrategy.Instance, Input(MomentumTestData.HighVolSqueezeBreakout()), p)!.Value.Setup;
    ...
    public void Qualifies_for_a_high_vol_squeeze_breakout_and_tags_it()
    {
        ...
        var built = TradePlanner.Build(MomentumStrategy.Instance, Input(MomentumTestData.HighVolSqueezeBreakout()), p)!.Value;

        built.Features.AboveTrend.Should().BeTrue();
        ((MomentumFeatures)built.Features).IsHighVolatility(p).Should().BeTrue();
        MomentumStrategy.Instance.Qualifies(built.Features, p).Should().BeTrue();
        built.Setup.Kind.Should().Be(SetupKind.Primary);
    }
    ...
    {
        var p = MomentumParams.Default;
        var built = TradePlanner.Build(MomentumStrategy.Instance, Input(MomentumTestData.LowVolBreakout()), p)!.Value;

        ((MomentumFeatures)built.Features).IsHighVolatility(p).Should().BeFalse();
        MomentumStrategy.Instance.Qualifies(built.Features, p).Should().BeFalse();
    }
    ...
    {
        var p = MomentumParams.Default;
        var built = TradePlanner.Build(MomentumStrategy.Instance, Input(MomentumTestData.Downtrend()), p)!.Value;

        ((MomentumFeatures)built.Features).AboveTrend.Should().BeFalse();
        MomentumStrategy.Instance.Qualifies(built.Features, p).Should().BeFalse();
    }
    ...
    {
        var p = MomentumParams.For(RiskLevel.High); // MaxBaseRangeAtr 4.5, MinAtrPercent 0.04, MinRelVol 1.3
        ...
        MomentumStrategy.IsSqueezeBreakout(tight, p).Should().BeTrue();
        MomentumStrategy.Instance.Qualifies(tight, p).Should().BeTrue();
        ...
        MomentumStrategy.IsSqueezeBreakout(loose, p).Should().BeFalse();
    }

    [Fact]
    public void High_risk_preset_targets_roughly_a_double_digit_move()
    {
        var p = MomentumParams.For(RiskLevel.High);
        var setup = TradePlanner.Build(MomentumStrategy.Instance, Input(MomentumTestData.HighVolSqueezeBreakout()), p)!.Value.Setup;
